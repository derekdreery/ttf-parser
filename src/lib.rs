...
use parser::{
    FromData, LazyArray16, LazyArrayIter16, NumFrom, Offset, Offset32, Stream, TryNumFrom,
};
...
#[cfg(feature = "std")]
type Bytes<'a> = std::borrow::Cow<'a, [u8]>;
#[cfg(not(feature = "std"))]
type Bytes<'a> = &'a [u8];
...
pub struct Face<'a> {
    ...
    // Safety: must be last, so it is dropped last.
    font_data: Bytes<'a>, // The input data. Used by Face::table_data.
}
...
impl<'a> Face<'a> {
    ...
    pub fn from_slice(data: &'a [u8], index: u32) -> Result<Self, FaceParsingError> {
        ...
        #[cfg(feature = "std")]
        let font_data = Cow::Borrowed(data);
        #[cfg(not(feature = "std"))]
        let font_data = data;
    ...
    /// Create a font face from owned data.
    #[cfg(feature = "std")]
    pub fn from_owned(
        data: std::vec::Vec<u8>,
        index: u32,
    ) -> Result<Face<'static>, FaceParsingError> {
        let face = Face::from_slice(&*data, index)?;
        // We're now going to unsafely extend the lifetime of `face` to `'static`, because we own
        // the backing slice and ensure it is dropped last. We could avoid this unsafe with
        // self-referential structs.
        unsafe {
            let mut face: Face<'static> = core::mem::transmute(face);
            face.font_data = Cow::Owned(data);
            Ok(face)
        }
    }

    fn collect_tables(data: &'a [u8], records: LazyArray16<TableRecord>) -> RawFaceTables<'a> {
    ...
    pub fn table_data(&self, tag: Tag) -> Option<&[u8]> {
        ...
        self.font_data.as_ref().get(offset..end)
    }
